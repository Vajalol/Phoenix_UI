local ProfileImport = Phoenix_UI:NewModule('Config.Components.ProfileImport');
...
function ProfileImport:ImportProfile(string, doImport)
    ...
    local valid, imported_Profile = Phoenix_UI:Deserialize(uncompressed_profile)
    if (doImport) then
        if valid and imported_Profile then
            for i, v in pairs(imported_Profile) do
                if type(v) == "table" then
                    Phoenix_UI.db.profile[i] = CopyTable(v)
                else
                    local new_value = v
                    Phoenix_UI.db.profile[i] = new_value
                end
            end
        else
            return "< ERROR: Invalid Import String >"
...
local function buildProfileImport()
    ...
    -- Create Window
    local window = Phoenix_UIConfig:Window(UIParent, 400, 325, 'Profile Import')
    ...
    -- Create EditBox
    local editBox = Phoenix_UIConfig:MultiLineBox(window, 380, 250, '< Paste Import String here >')
    Phoenix_UIConfig:GlueTop(editBox, window, 0, -40)

    editBox.editBox:SetFocus()
    editBox.editBox:HighlightText()

    -- Create Import Button
    local importButton = Phoenix_UIConfig:Button(window, 65, 20, 'Validate')
    Phoenix_UIConfig:GlueBelow(importButton, window, -35, 30)

    -- Create Close Button
    local closeButton = Phoenix_UIConfig:Button(window, 65, 20, 'Close')
    Phoenix_UIConfig:GlueBelow(closeButton, window, 35, 30)
    ...
    importButton:SetScript("OnClick", function(self)
        ...
        if (ProfileImport:ImportProfile(editBox:GetValue())) then
            ...
            elseif (text == 'Import') then
                ...
                Phoenix_UIConfig:Confirm('Import Profile', 'This will overwrite your existing settings!', buttons)
            end
        end
    end)
end
