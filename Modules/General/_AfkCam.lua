local Module = Phoenix_UI:NewModule("General.AfkCam");

function Module:OnEnable()
    local db = Phoenix_UI.db.profile.general.cosmetic.afkscreen
    if (db) then
        ...
        local blank = [[Interface\Phoenix_UI\Media\Textures\Core\blank.tga]]
        local normTex = [[Interface\Phoenix_UI\Media\Textures\Core\normTex.tga]]
        ...
        AFKPanel.Text:SetText("|cffFF7D0APhoenix|r|cffFF0000_|r|cffFFD100UI|r")
        ...
        AFKPanel:SetScript("OnShow", function(self)
            ...
            if not InCombatLockdown() then
                if PartyFrame then
                    PartyFrame:SetAttribute("state-visibility", "hide")
                end
                if CompactRaidFrameContainer then
                    CompactRaidFrameContainer:SetAttribute("state-visibility", "hide")
                end
            end
        end)
        ...
        AFKPanel:SetScript("OnHide", function(self)
            ...
            if not InCombatLockdown() then
                if PartyFrame then
                    PartyFrame:SetAttribute("state-visibility", "show")
                end
                if CompactRaidFrameContainer then
