local Module = Phoenix_UI:NewModule("General.Fonts");

function Module:OnInitialize()
    -- Check both old and new font configuration locations
    local legacyFont = Phoenix_UI.db.profile.general.font
    local newFontSystem = Phoenix_UI.db.profile and Phoenix_UI.db.profile.fonts and Phoenix_UI.db.profile.fonts.gameFont
    local font = newFontSystem and newFontSystem.family or legacyFont
    
    -- Only proceed if we have a font and it's different from standard
    if font and (font ~= STANDARD_TEXT_FONT) then
        ...
        -- If we're using the legacy system, make sure the new system is updated
        if legacyFont and (not newFontSystem or newFontSystem.family ~= legacyFont) then
            -- Create the fonts structure if it doesn't exist
            Phoenix_UI.db.profile.fonts = Phoenix_UI.db.profile.fonts or {}
            Phoenix_UI.db.profile.fonts.gameFont = Phoenix_UI.db.profile.fonts.gameFont or {}
            Phoenix_UI.db.profile.fonts.gameFont.family = legacyFont
        end
    end
end
