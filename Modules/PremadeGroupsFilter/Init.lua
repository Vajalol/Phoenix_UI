...
-- Set up compatibility variables to maintain backward compatibility
...
function PGF.MigrateStateV4()
    if PremadeGroupsFilterState and PremadeGroupsFilterState.version and PremadeGroupsFilterState.version < 4 then
...
function PGF.MigrateStateV5()
    if PremadeGroupsFilterState and PremadeGroupsFilterState.version and PremadeGroupsFilterState.version < 5 then
        local mapping = {
...
function PGF.MigrateStateV6()
    if PremadeGroupsFilterState and PremadeGroupsFilterState.version and PremadeGroupsFilterState.version < 6 then
        for k, v in pairs(PremadeGroupsFilterState) do
...
function PGF.MigrateStateV7()
    if PremadeGroupsFilterState and PremadeGroupsFilterState.version and PremadeGroupsFilterState.version < 7 then
...
function PGF.OnAddonLoaded(name)
    if name == PGFAddonName then
        -- Try to use the new shared database if Phoenix_UI is available
        if Phoenix_UI and Phoenix_UI.moduleDB and Phoenix_UI.moduleDB.PremadeGroupsFilter then
            ...
            -- Initialize PGF.State to avoid nil reference
            PGF.State = {version = 7}
            PremadeGroupsFilterState = PGF.State
            
            -- Use namespace from Phoenix_UI per-character DB if available
            if Phoenix_UI.moduleCharDB and Phoenix_UI.moduleCharDB.PremadeGroupsFilter and Phoenix_UI.moduleCharDB.PremadeGroupsFilter.profile then
                ...
                if not PGF.State.version then
                    PGF.State = {
                        version = 7, -- Use latest version to skip migrations
            ...
            PGF.Settings = PremadeGroupsFilterSettings
            ...
            PGF.State = PremadeGroupsFilterState
            
            -- Try again after a delay - Phoenix_UI might load later
            C_Timer.After(2, function()
                if Phoenix_UI and Phoenix_UI.moduleDB and Phoenix_UI.moduleDB.PremadeGroupsFilter then
                    -- Copy our settings to the namespace 
                    Phoenix_UI.moduleDB.PremadeGroupsFilter.profile = CopyTable(PGF.Settings)
                    PGF.Settings = Phoenix_UI.moduleDB.PremadeGroupsFilter.profile
                    PremadeGroupsFilterSettings = PGF.Settings
