---@class PhoenixUI_ChatModule : AceModule
local Module = Phoenix_UI:NewModule("Chat.Core");

---@param force boolean? Force reinitialize settings even if they exist
---@return table chat settings table
function Module:EnsureChatSettings(force)
    if not Phoenix_UI.db or not Phoenix_UI.db.profile then
        return {}
    end
    
    if not Phoenix_UI.db.profile.chat or force then
        Phoenix_UI.db.profile.chat = {}
    end
    
    local chat = Phoenix_UI.db.profile.chat
    
    -- Initialize standard chat settings if they don't exist
    if not chat.history or force then
        chat.history = { enabled = true, lines = 500 }
    end
    
    if not chat.expandedView or force then
        chat.expandedView = { enabled = true }
    end
    
    if not chat.classSpec or force then
        chat.classSpec = { enabled = true, showIcons = true }
    end
    
    -- Save changes if requested
    if force and Phoenix_UI.SaveDB then
        Phoenix_UI:SaveDB()
    end
...
---@param msg string Message to process
---@return string Processed message with emojis, URLs, etc.
function Module:ProcessChatMessage(msg)
    if not msg or msg == "" then return msg end
    
    -- Apply registered handlers in specific order
    local result = msg
    
    -- Safely run handlers with pcall
    local function SafeProcess(handlerName, text)
        if not Phoenix_UI.ChatEmojiHandlers or not Phoenix_UI.ChatEmojiHandlers[handlerName] then
            return text
        end
        
        local success, processed = pcall(Phoenix_UI.ChatEmojiHandlers[handlerName], text)
        if success then
            return processed or text
        else
            -- Log error but don't break chat
            if Phoenix_UI.debug then
                print("|cffFF7D0APhoenix UI:|r Error in chat handler '" .. handlerName .. "': " .. tostring(processed))
            end
            return text
        end
    end
    
    -- Process emojis first if enabled
    local db = self:EnsureChatSettings()
    if db.emoji and db.emoji.enabled then
        result = SafeProcess("emoji", result)
    end
    
    -- Process URLs if enabled
    if db.urls and db.urls.enabled then
        result = SafeProcess("urls", result)
    end
...
function Module:OnInitialize()
    -- Create global handler table if it doesn't exist
    Phoenix_UI.ChatEmojiHandlers = Phoenix_UI.ChatEmojiHandlers or {}
    
    -- Initialize settings
    self:EnsureChatSettings()
end
...
function Module:OnEnable()
    ...
    -- Register message filter for chat frames
    ChatFrame_AddMessageEventFilter("CHAT_MSG_CHANNEL", function(_, _, msg, ...)
        -- Only process if we have handlers and the message doesn't contain item links
        if msg and not msg:find("|c%x+|Hitem:") then
            return false, self:ProcessChatMessage(msg), ...
        end
        return false, msg, ...
    end)
    
    -- Add filters for other chat types
    local chatTypes = {
        "SAY", "YELL", "GUILD", "OFFICER", "PARTY", "RAID", "RAID_LEADER",
        "RAID_WARNING", "INSTANCE_CHAT", "INSTANCE_CHAT_LEADER", "WHISPER",
        "WHISPER_INFORM", "EMOTE", "BN_WHISPER", "BN_WHISPER_INFORM"
    }
    
    for _, chatType in ipairs(chatTypes) do
        ChatFrame_AddMessageEventFilter("CHAT_MSG_" .. chatType, function(_, _, msg, ...)
            -- Only process if we have handlers and the message doesn't contain item links
            if msg and not msg:find("|c%x+|Hitem:") then
                return false, self:ProcessChatMessage(msg), ...
            end
