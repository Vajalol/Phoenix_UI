...
function Module:OnEnable()
    ...
    -- Initialize social integration settings if not present
    if not db.social then
        db.social = {
            enabled = true,
            enhancedStatuses = true,
            guildRanks = true,
            friendNotes = true,
            inlineTooltips = true
        }
    ...
    -- Exit if social integration is disabled
    if not db.social.enabled then return end
    ...
    if Phoenix_UI.debug then
        print("|cffFF7D0APhoenix UI:|r Enhanced social integration enabled.")
    end
end 