local Module = Phoenix_UI:NewModule("UnitFrames.Colors")
...
function Module:OnEnable()
    ...
    local db = Phoenix_UI.db.profile.unitframes
    local texture = Phoenix_UI.db.profile.general.texture
    if (db) then
        if (db.classcolor) then
            function Phoenix_UIUnitColor(healthbar, unit)
            ...
            hooksecurefunc("UnitFrameHealthBar_Update", function(self)
                Phoenix_UIUnitColor(self, self.unit)
            end)
