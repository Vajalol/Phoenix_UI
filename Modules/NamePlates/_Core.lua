local Module = Phoenix_UI:NewModule("NamePlates.Core");
...
function Module:OnEnable()
    ...
    local focusTexture = [[Interface\Phoenix_UI\Media\Textures\Nameplates\focusTexture]]

    -- NPC Colors Table
    Phoenix_UI_NPCColors = {}

    -- Insert NPC Colors with Keyvalues
    for _, npc in pairs(db.npccolors) do
        Phoenix_UI_NPCColors[npc.id] = npc.color
    end
    ...
    local function iconSkin(icon, parent)
        ...
        --border:SetVertexColor(unpack(Phoenix_UI:Color()))
        ...
        if Phoenix_UI:Color() then
            back:SetBackdropBorderColor(unpack(Phoenix_UI:Color(0.25)))
        end
        back:SetAlpha(0.9)
        icon.bg = back
        icon.styled = true
            ...
            if self.healthBar and self.unit then
                if UnitName("player") ~= UnitName(self.unit) then
                    ...
                    if db.colors then
                        ...
                        local color = Phoenix_UI_NPCColors[tonumber(id)] or { r = 0, g = 1, b = 0.6, a = 1 }
                        local nColor = Phoenix_UI_NPCColors[tonumber(id)] or { r = 1, g = 0, b = 0.3, a = 1 }
