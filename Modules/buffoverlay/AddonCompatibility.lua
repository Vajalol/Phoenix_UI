...
-- Add frameName cache for faster lookups
local frameNameCache = {}
    ...
    ["ElvUI"] = {
        {
            ...
            type = "raid",
            unit = "unit",
            priority = 1, -- Higher priority for healer frames
        },
        {
...
-- Add new optimization function for healing addons
function BuffOverlay:OptimizeHealingAddonDetection()
    -- Directly check common healer AddOns
    for addonName, frames in pairs(addonFrameInfo) do
        if IsAddOnLoaded(addonName) then
            -- Pre-scan common frame patterns for healing addons
            if addonName == "VuhDo" then
                -- For VuhDo, directly check known panel structures
                for panelNum = 1, 8 do -- VuhDo typically uses up to 8 panels
                    for buttonNum = 1, 40 do -- Up to 40 raid members
                        -- Common VuhDo frame naming patterns
                        local frameName = "Vd" .. panelNum .. "H" .. buttonNum
                        local frame = _G[frameName]
                        if frame and frame.raidid then
                            self:RegisterUnitFrame(frame, "raid", frame.raidid)
                            -- Cache the frame name for faster lookups
                            frameNameCache[frameName] = frame
                        end
                    end
                end
            elseif addonName == "HealBot" then
                -- HealBot optimization
                for buttonNum = 1, 50 do -- HealBot supports many buttons
                    local frameName = "HealBot_Action_HealUnit" .. buttonNum
                    local frame = _G[frameName]
                    if frame and frame.unit then
                        self:RegisterUnitFrame(frame, "raid", frame.unit)
                        -- Cache the frame name
                        frameNameCache[frameName] = frame
                    end
                end
            elseif addonName == "ElvUI" then
                -- ElvUI optimization
                -- Check for raid frames
                for group = 1, 8 do
                    for member = 1, 5 do
                        local frameName = "ElvUF_Raid" .. group .. "Group" .. group .. "UnitButton" .. member
                        local frame = _G[frameName]
                        if frame and frame.unit then
                            self:RegisterUnitFrame(frame, "raid", frame.unit)
                            -- Cache the frame name
                ...
                for member = 1, 5 do
                    local frameName = "ElvUF_PartyGroup1UnitButton" .. member
                    local frame = _G[frameName]
                    if frame and frame.unit then
                        self:RegisterUnitFrame(frame, "party", frame.unit)
                        -- Cache the frame name
                        frameNameCache[frameName] = frame
                    end
                end
            elseif addonName == "Grid2" then
                -- Grid2 optimization - it uses a layout system
                if Grid2 and Grid2.frames then
                    for frame in pairs(Grid2.frames) do
                        if frame.unit then
                            self:RegisterUnitFrame(frame, "raid", frame.unit)
                        end
                    end
            ...
            -- Prioritize these patterns for ongoing scans
            for _, frameInfo in pairs(frames) do
                if frameInfo.priority and frameInfo.priority > 0 then
                    enabledPatterns[frameInfo.frame] = frameInfo.type
                end
            end
        end
    end
...
-- Optimize frame registration
local originalRegisterUnitFrame = BuffOverlay.RegisterUnitFrame
BuffOverlay.RegisterUnitFrame = function(self, frame, frameType, unit)
    -- Skip if already registered
    if frame.BuffOverlayRegistered then
        return
    end
    
    -- Mark as registered to avoid duplicates
    frame.BuffOverlayRegistered = true
    
    -- Call original function
    originalRegisterUnitFrame(self, frame, frameType, unit)
    
    -- If this is a named frame, cache it
...
-- Add a function to optimize frame lookup by name
function BuffOverlay:GetFrameByName(frameName)
    -- First check cache for direct lookup
    if frameNameCache[frameName] then
        return frameNameCache[frameName]
    end
...
-- Add hook to PLAYER_ENTERING_WORLD to optimize healing addon detection
local originalFindFrames = BuffOverlay.FindFrames
BuffOverlay.FindFrames = function(self)
    -- Call original function
    originalFindFrames(self)
    
    -- Also optimize healing addon detection
