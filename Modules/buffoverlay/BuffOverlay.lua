...
-- Optimized texture lookup for frequently used textures
local function GetCachedTexture(spellId)
    if not spellId then return nil end
    
    if textureCache[spellId] then
...
local function InsertTestBuff(spellId)
    local tex = GetCachedTexture(spellId)
...
local function ValidateBuffData()
    ...
    if not BuffOverlay.db or not BuffOverlay.db.profile or not BuffOverlay.db.profile.buffs then
        return
    end
    
    -- Ensure defaultSpells exists
    if not BuffOverlay.defaultSpells then 
        BuffOverlay.defaultSpells = {}
        return
    end
    
    -- Ensure global tables exist
    if not BuffOverlay.db.global then BuffOverlay.db.global = {} end
    if not BuffOverlay.db.global.customBuffs then BuffOverlay.db.global.customBuffs = {} end
        ...
        if (not BuffOverlay.defaultSpells[k]) and (not BuffOverlay.db.global.customBuffs[k]) then
            ...
            if v.parent then -- child found
                ...
                if BuffOverlay.defaultSpells[k] and not BuffOverlay.defaultSpells[k].parent then
                    ...
                    -- Check if parent exists in the buffs table
    ...
    if BuffOverlay.UpdateCustomBuffs then
...
local function HideAllOverlays(frame)
    if not frame or not frame.BuffOverlays then return end
    
    local children = { frame.BuffOverlays:GetChildren() }
...
local function ValidateSpellIds()
    -- Check for defaultSpells property
    if not BuffOverlay.defaultSpells then
        BuffOverlay.defaultSpells = {}
        return
    end
    ...
    for spellId in pairs(BuffOverlay.defaultSpells) do
        if type(spellId) == "number" then
            if not C_Spell.DoesSpellExist(spellId) then
                ...
                if BuffOverlay.db and BuffOverlay.db.profile and BuffOverlay.db.profile.buffs then
                    BuffOverlay.db.profile.buffs[spellId] = nil
                end
                
                if BuffOverlay.db and BuffOverlay.db.global and BuffOverlay.db.global.customBuffs then
    ...
    for spellId in pairs(BuffOverlay.db.profile.buffs) do
        if type(spellId) == "number" then
            if not C_Spell.DoesSpellExist(spellId) then
                ...
                if BuffOverlay.db.global and BuffOverlay.db.global.customBuffs then
                    BuffOverlay.db.global.customBuffs[spellId] = nil
                end
    ...
    -- Make sure global.customBuffs exists
    if not BuffOverlay.db.global or not BuffOverlay.db.global.customBuffs then
        if not BuffOverlay.db.global then BuffOverlay.db.global = {} end
        BuffOverlay.db.global.customBuffs = {}
        return
    end
...
local function ValidateDatabase()
    ...
    -- Check if profiles table exists before iterating over it
    if BuffOverlay.db and BuffOverlay.db.profiles then
    ...
    -- Make sure global table exists
    if not BuffOverlay.db.global then
        BuffOverlay.db.global = {}
    end
...
function BuffOverlay:OnInitialize()
    -- Try to use Phoenix_UI database if available
    if Phoenix_UI and Phoenix_UI.moduleDB and Phoenix_UI.moduleDB.BuffOverlay then
        self.db = Phoenix_UI.moduleDB.BuffOverlay
        
        -- Initialize basic DB structure if needed
        ...
        if not self.db.profile.bars then self.db.profile.bars = {} end
        if not self.db.profile.minimap then self.db.profile.minimap = { hide = false } end
        if not self.db.global.customBuffs then self.db.global.customBuffs = {} end
        
        -- If this is our first run with Phoenix_UI, validate and migrate
        if not self.db.profile.phoenixUIIntegrated then
            ValidateDatabase()
            ValidateSpellIds()
            ValidateBarAttributes()
            self.db.profile.phoenixUIIntegrated = true
        end
    else
        -- Fallback to own database if Phoenix_UI not available
        ...
        -- Set up a delay to check again for Phoenix_UI
        C_Timer.After(2, function()
            if Phoenix_UI and Phoenix_UI.moduleDB and Phoenix_UI.moduleDB.BuffOverlay then
                -- Copy our current settings to Phoenix_UI
                Phoenix_UI.moduleDB.BuffOverlay.profile = CopyTable(self.db.profile)
                Phoenix_UI.moduleDB.BuffOverlay.global = CopyTable(self.db.global)
                Phoenix_UI.moduleDB.BuffOverlay.profile.phoenixUIIntegrated = true
                
                -- Use Phoenix_UI database from now on
                self.db = Phoenix_UI.moduleDB.BuffOverlay
                print("|cffFF7D0APhoenix|r|cffFF0000_|r|cffFFD100UI|r: BuffOverlay integrated with central database.")
            end
        end)
    end
    ...
    -- Only show welcome message if explicitly enabled by user profile AND not integrated with Phoenix UI
    local isIntegrated = (self.db.profile.phoenixUIIntegrated or (Phoenix_UI and Phoenix_UI.moduleDB and Phoenix_UI.moduleDB.BuffOverlay))
    if self.db.profile.welcomeMessage and not isIntegrated then
        self:Print(format(L[" %s BuffOverLay is Active."], self:Colorize("Open |cffFF7D0APhoenix|r|cffFF0000_|r|cffFFD100UI|r Panel For More Options /buffoverlay", "accent")))
    end
    ...
    BuffOverlay.customIcons = BuffOverlay.customIcons or {}
    BuffOverlay.ignoreParentIcons = BuffOverlay.ignoreParentIcons or {}
...
function BuffOverlay:RefreshOverlays(full, barName)
    ...
    -- Initialize buffs and bars if they don't exist
    if not self.db.profile.buffs then self.db.profile.buffs = {} end
    if not self.db.profile.bars then self.db.profile.bars = {} end
    ...
    if full then
        if self.overlays then  -- Check if overlays exists
            for _, overlay in pairs(self.overlays) do
                if barName then
                    if overlay.bar and overlay.bar.id == barName then
                        if overlay.StopAllGlows then overlay:StopAllGlows() end
                        overlay:Hide()
    ...
    -- Initialize unitFrames if it doesn't exist
    if not self.unitFrames then self.unitFrames = {} end
    
    for unit, frames in pairs(self.unitFrames) do
        if frames then  -- Check if frames exists
            for frame in pairs(frames) do
                if frame and frame.IsShown and frame:IsShown() then
                    self:ApplyOverlay(frame, unit, barName)
                else
                    if frame then HideAllOverlays(frame) end
                end
            end
        end
    end

    -- Initialize blizzFrames if it doesn't exist
    if not self.blizzFrames then self.blizzFrames = {} end
...
local function HideTestFrames()
    ...
    elseif CompactRaidFrameManager and GetNumGroupMembers() == 0 then
        -- Use SetAttribute for protected frames to avoid taint issues
        if not InCombatLockdown() then
            -- SetAttribute is safe while not in combat
            if CompactRaidFrameManager then
                CompactRaidFrameManager:SetAttribute("state-visibility", "hide")
            end
            -- Never directly hide these frames
            if CompactPartyFrame then
                CompactPartyFrame:SetAttribute("state-visibility", "hide")
            end
        else
            -- Register for combat end to handle this safely
            local combatFrame = combatDropUpdate or CreateFrame("Frame")
            combatFrame.hideFunc = function()
                if CompactRaidFrameManager then
                    CompactRaidFrameManager:SetAttribute("state-visibility", "hide")
                end
                if CompactPartyFrame then
...
combatDropUpdate:SetScript("OnEvent", function(self)
    HideTestFrames()
    -- Call the hideFunc if it exists
        ...
        if GetNumGroupMembers() == 0 then
            if CompactRaidFrameManager then
                ...
                if not InCombatLockdown() then
                    ...
                    if CompactPartyFrame then
                        CompactPartyFrame:SetAttribute("state-visibility", "show")
                        if PartyFrame and PartyFrame.UpdatePaddingAndLayout then
                            PartyFrame:UpdatePaddingAndLayout()
                        end
                    end
                else
                    -- Queue this for after combat
                    local combatFrame = CreateFrame("Frame")
                    combatFrame:RegisterEvent("PLAYER_REGEN_ENABLED")
                    combatFrame:SetScript("OnEvent", function(eventFrame)
                        if CompactRaidFrameManager then
                            CompactRaidFrameManager:SetAttribute("state-visibility", "show")
                        end
                        if CompactRaidFrameContainer then
...
function BuffOverlay:ApplyOverlay(frame, unit, barNameToApply)
                    ...
                    if bar.showTooltip and not overlay:GetScript("OnEnter") then
                        overlay:SetScript("OnEnter", function(s)
                            ...
                            if self.test then
                                ...
                                -- Safe use of SetUnitAura
                                if s.unit and s:GetID() and s.filter then
                                    GameTooltip:SetUnitAura(s.unit, s:GetID(), s.filter)
                                elseif s.spellId then
                                    -- Fallback to spell ID if we can't use SetUnitAura properly
                                    GameTooltip:SetSpellByID(s.spellId)
                                end
                            end
                        end)
    ...
    for _, filter in ipairs(filters) do
        for i = 1, 40 do
            local spellName, icon, count, dispelType, duration, expirationTime, source, _, _, spellId
            
            -- Use the module's own UnitAura wrapper that already supports the modern API
            ...
            if auraData then
                if type(auraData) == "table" then
                    ...
                    spellName, icon, count, dispelType, duration, expirationTime, source, _, _, spellId = auraData, icon, count, dispelType, duration, expirationTime, source, _, _, spellId
                end
            else
                break
            end
...
-- In the OnEnable function, after the existing initialization code
function BuffOverlay:OnEnable()
    -- Existing initialization code
    
    -- Register for role change events - fixed to use methods instead of direct AceEvent calls
    ...
    -- Call role check on initial load
    self:CheckPlayerRole()
...
-- Add the role detection function
function BuffOverlay:CheckPlayerRole()
    -- Detect if player is a healer
    ...
    self.playerIsHealer = isHealer
    
    -- Apply healer-specific optimizations if needed
    if isHealer and not self.healerOptimized then
        self:EnhanceForHealers()
        self.healerOptimized = true
    end
end

-- Add the healer optimization function
function BuffOverlay:EnhanceForHealers()
    for barName, bar in pairs(self.db.profile.bars) do
        -- Only apply once per bar
        if not bar.healer_optimized then
            -- Enhance visibility of important effects
            bar.iconScale = math.min(bar.iconScale * 1.15, 1.5)
            bar.stackCountScale = math.min(bar.stackCountScale * 1.1, 1.2)
            
            -- Improve visibility of dispellable effects
            bar.debuffIconBorderColorByDispelType = true
            bar.iconBorderSize = math.min(bar.iconBorderSize + 1, 3)
            
            -- Show cooldown spirals for better timing awareness
            bar.showCooldownSpiral = true
            
            -- Show stack counts
            bar.showStackCount = true
            
            -- Enable frame types most relevant to healers
            if bar.frameTypes then
                bar.frameTypes["raid"] = true
                bar.frameTypes["party"] = true
            end
            
            -- Ensure visibility in relevant PvE content
            bar.showInRaid = true
            bar.showInDungeon = true
            
            -- Track that we've optimized this bar
            bar.healer_optimized = true
        end
    end
...
function BuffOverlay:OptimizePerformance()
    -- Add texture cache for spell icons
    local textureCache = {}
    local GetCachedTexture = function(spellId)
    ...
    -- Add throttling for frame updates based on group size
    local lastUpdateTime = 0
    local originalRefreshOverlays = self.RefreshOverlays
    
    -- Replace with optimized version that includes throttling
    self.RefreshOverlays = function(self, force)
        local currentTime = GetTime()
        
        -- Always allow forced refreshes
        if force then
            lastUpdateTime = currentTime
            return originalRefreshOverlays(self, force)
        end
        
        -- Calculate appropriate update interval based on group size
        local interval = 0.2 -- 200ms base update interval
        local numGroupMembers = GetNumGroupMembers()
        
        if numGroupMembers > 20 then -- Raid environment
            interval = 0.3 -- 300ms in large groups
        elseif numGroupMembers > 10 then -- Small raid
            interval = 0.25 -- 250ms in medium groups
        end
        
        -- Only update if enough time has passed or we're not in combat
        if (currentTime - lastUpdateTime) < interval and InCombatLockdown() then
            return -- Skip this update
        end
        
        -- Track time of this update
        lastUpdateTime = currentTime
        
        -- Call original refresh function
        originalRefreshOverlays(self, force)
    end
    
    -- Replace texture lookup with cached version in SetOverlayAura
    local originalSetOverlayAura = SetOverlayAura
    SetOverlayAura = function(overlay, index, icon, count, duration, expirationTime, dispelType, filter, spellId)
        -- Use cached texture if available and no icon was provided
        if spellId and not icon and spellId > 0 then
            icon = GetCachedTexture(spellId)
        end
        
        -- Call original function
        originalSetOverlayAura(overlay, index, icon, count, duration, expirationTime, dispelType, filter, spellId)
    end
    
    -- Optimize UpdateOverlays to skip processing on hidden frames
    local originalUpdateOverlays = self.UpdateOverlays
    self.UpdateOverlays = function(self, frame, unit)
        -- Skip processing for frames that aren't visible
        if not frame:IsVisible() then
            return
        end
        
        -- Call original function
        originalUpdateOverlays(self, frame, unit)
    end
end
