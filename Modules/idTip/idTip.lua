...
-- Initialize configuration with Phoenix_UI if available
local idTipConfig
if Phoenix_UI and Phoenix_UI.db then
    -- Register a namespace for idTip in Phoenix_UI
    if not Phoenix_UI.moduleDB.idTip then
        Phoenix_UI.moduleDB.idTip = Phoenix_UI.db:RegisterNamespace("idTip", {
            profile = {
                enabled = true,
                -- Set default enabled kinds
                spellEnabled = true,
                itemEnabled = true,
                unitEnabled = true,
                questEnabled = true,
                ...
                macroEnabled = true,
                setEnabled = true,  -- For Set items (Tier sets, etc.), not a function name
                visualEnabled = true,
                sourceEnabled = true,
                speciesEnabled = true,
                iconEnabled = true,
    ...
    -- Use the Phoenix_UI namespace
    idTipConfig = Phoenix_UI.moduleDB.idTip.profile
    
    -- If we're coming from a standalone version, migrate settings
    if _G.idTipConfig and not idTipConfig.migratedFromOldDB then
        for k, v in pairs(_G.idTipConfig) do
            idTipConfig[k] = v
        end
        idTipConfig.migratedFromOldDB = true
        print("|cffFF7D0APhoenix|r|cffFF0000_|r|cffFFD100UI|r: idTip settings migrated to central database.")
    end
    
    -- Make the global variable reference our namespace for other addons
    _G.idTipConfig = idTipConfig
end
...
-- This table is accessed by the configuration panel
_G.kinds = {
  spell = "SpellID",
  item = "ItemID",
  unit = "NPC ID",
...
-- Helper function to get config key for a kind
...
local function addLine(tooltip, id, kind)
  ...
  if _G.idTipConfig and (not _G.idTipConfig.enabled or not _G.idTipConfig[configKey(kind)]) then return end
...
if UnitAura then
  hook(GameTooltip, "SetUnitAura", function(tooltip, ...)
    -- Use modern C_UnitAuras API for WoW 10.0+
    local unit, index, filter = ...
    local id
    
    if C_UnitAuras and C_UnitAuras.GetAuraDataByIndex then
        -- Safe access with error handling
        local success, auraData = pcall(function() return C_UnitAuras.GetAuraDataByIndex(unit, index, filter) end)
        id = success and auraData and auraData.spellId
    else
        -- Fallback for Classic
        id = select(10, UnitAura(...))
    end
    
    if id then addLine(tooltip, id, "spell") end
  end)
end
...
f:SetScript("OnEvent", function(_, _, addon)
  if addon == addonName then
    ...
    if not _G.idTipConfig then _G.idTipConfig = {} end

    for key, _ in pairs(defaults) do
      if type(_G.idTipConfig[key]) ~= type(defaults[key]) then _G.idTipConfig[key] = defaults[key] end
    end

    for key, _ in pairs(kinds) do
      local configKeyName = configKey(key)
      if type(_G.idTipConfig[configKeyName]) ~= "boolean" then
        _G.idTipConfig[configKeyName] = not contains(defaultDisabledKinds, key)
      end
    end
...
panel:SetScript("OnShow", function()
  local function createCheckbox(label, key)
    ...
    checkBox:SetChecked(_G.idTipConfig[key])
    checkBox:HookScript("OnClick", function(self)
      local checked = self:GetChecked()
      _G.idTipConfig[key] = checked
    end)
