local Debuffs = Phoenix_UI:NewModule("Buffs.Debuffs");

function Debuffs:OnEnable()
    if C_AddOns.IsAddOnLoaded("BlizzBuffsFacade") then return end

    local db = Phoenix_UI.db.profile.unitframes.buffs
    local theme = Phoenix_UI.db.profile.general.theme
    ...
    local function ButtonDefault(button)
        ...
        border.texture:SetTexture("Interface\\Phoenix_UI\\Media\\Textures\\Core\\gloss_border_w")
        ...
        -- Create direct borderColor
        border.shadow:SetBackdropBorderColor(0.5, 0.5, 0.5, 1)
    ...
    function updateDebuffs()
        ...
        for index, child in pairs(Children) do
            ...
            if frame.Phoenix_UIBorder == nil then
                ButtonDefault(frame)
            end
            ...
            if (frame.Phoenix_UIBorder) then
                local color
                if (debuffType) then
                ...
                if color ~= nil then
                    ...
                    -- Set a fixed color without relying on Phoenix_UI:Color
                    frame.Phoenix_UIBorder.texture:SetVertexColor(0.5, 0.5, 0.5, 1)
                end
            end
        end
    end
