...
local function SetOptionUserDisabled(isDisabled)
	-- Only proceed if savedVariables exists
...
local function IsModDisabled()
	-- Make sure savedVariables exists before we check it
...
local function InitSavedVariables()
	-- Check if we should use Phoenix_UI database
	local usePhoenix = (Phoenix_UI and Phoenix_UI.moduleDB and Phoenix_UI.moduleDB.MSBT)
	
	-- Add safety checks to prevent errors on first load
	if usePhoenix then
		-- Initialize profile data
		if not Phoenix_UI.moduleDB.MSBT.profile then 
			Phoenix_UI.moduleDB.MSBT.profile = {}
		end
		
		-- Initialize profile.profiles if it doesn't exist
		if not Phoenix_UI.moduleDB.MSBT.profile.profiles then
			Phoenix_UI.moduleDB.MSBT.profile.profiles = {}
		end
		
		-- Initialize saved media
		if not Phoenix_UI.moduleDB.MSBTMedia or not Phoenix_UI.moduleDB.MSBTMedia.profile then
			Phoenix_UI.moduleDB.MSBTMedia = Phoenix_UI.moduleDB.MSBTMedia or {}
			Phoenix_UI.moduleDB.MSBTMedia.profile = Phoenix_UI.moduleDB.MSBTMedia.profile or {}
		end
		
		-- Initialize fonts and sounds
		Phoenix_UI.moduleDB.MSBTMedia.profile.fonts = Phoenix_UI.moduleDB.MSBTMedia.profile.fonts or {}
		Phoenix_UI.moduleDB.MSBTMedia.profile.sounds = Phoenix_UI.moduleDB.MSBTMedia.profile.sounds or {}
		
		-- Set up globals for backward compatibility
		MSBTProfiles_SavedVars = Phoenix_UI.moduleDB.MSBT.profile
		MSBT_SavedMedia = Phoenix_UI.moduleDB.MSBTMedia.profile
	else
		-- Traditional initialization with safety checks
		if (not MSBTProfiles_SavedVars) then MSBTProfiles_SavedVars = {} end
		if (not MSBTProfiles_SavedVars.profiles) then MSBTProfiles_SavedVars.profiles = {} end
		if (not MSBT_SavedMedia) then MSBT_SavedMedia = {} end
		if (not MSBT_SavedMedia.fonts) then MSBT_SavedMedia.fonts = {} end
		if (not MSBT_SavedMedia.sounds) then MSBT_SavedMedia.sounds = {} end
	end
	...
	if not savedVariables.profiles[DEFAULT_PROFILE_NAME] then
		savedVariables.profiles[DEFAULT_PROFILE_NAME] = {}
		savedVariables.profiles[DEFAULT_PROFILE_NAME].creationVersion = MikSBT.VERSION .. "." .. MikSBT.SVN_REVISION
	end

	-- Ensure currentProfileName is valid
	if not savedVariablesPerChar.currentProfileName or not savedVariables.profiles[savedVariablesPerChar.currentProfileName] then
		savedVariablesPerChar.currentProfileName = DEFAULT_PROFILE_NAME
	end

	-- Select the current profile for the character
	SelectProfile(savedVariablesPerChar.currentProfileName)

	-- Ensure currentProfile is set (extra safety check)
	if not currentProfile then
		currentProfile = savedVariables.profiles[DEFAULT_PROFILE_NAME]
		module.currentProfile = currentProfile
	end
	...
	if not savedMedia then
		...
		-- Ensure font and sounds tables exist
		if not savedMedia.fonts then savedMedia.fonts = {} end
		if not savedMedia.sounds then savedMedia.sounds = {} end
	end
	...
	-- Migrate data from old system to new if needed
	if Phoenix_UI and Phoenix_UI.moduleDB and Phoenix_UI.moduleDB.MSBT and not savedVariables.migratedFromOldDB then
		-- Check for old data outside of Phoenix_UI
		local oldVariables = _G[SAVED_VARS_NAME]
		if oldVariables and oldVariables ~= savedVariables and oldVariables.profiles then
			-- Copy all profiles from old database
			for profileName, profileData in pairs(oldVariables.profiles) do
				savedVariables.profiles[profileName] = CopyTable(profileData)
			end
			
			-- Copy any other top-level settings
			for key, value in pairs(oldVariables) do
				if key ~= "profiles" and type(value) ~= "table" then
					savedVariables[key] = value
				end
			end
			
			savedVariables.migratedFromOldDB = true
			print("|cffFF7D0APhoenix|r|cffFF0000_|r|cffFFD100UI|r: MSBT settings migrated to central database.")
		end
	end
	
	-- Final check to ensure savedMedia exists and is properly initialized
	if not savedMedia then
		savedMedia = {}
	end
	
	-- Ensure savedMedia has the required tables
	if not savedMedia.fonts then savedMedia.fonts = {} end
	if not savedMedia.sounds then savedMedia.sounds = {} end
	
	-- Make sure module has access to saved media
	module.savedMedia = savedMedia
	
	-- Double-check that the module has access to the currentProfile
	if not module.currentProfile then
		module.currentProfile = currentProfile
	end
end
...
local function OnEvent(this, event, arg1)
	...
	if (event == "ADDON_LOADED") then
		-- Check for Phoenix_UI since MSBT is loaded as part of it
		if (arg1 ~= "Phoenix_UI") then return end
