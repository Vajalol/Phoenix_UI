...
local ColorPickerMethods = {
	SetColorRGBA = function(self, r, g, b, a)
		-- Validate and set default values for any nil or invalid components
...
--- alphaSliderTexture = [[Interface\AddOns\YourAddon\Libs\Phoenix_UIConfig\media\Checkers.tga]]
function Phoenix_UIConfig:ColorPicker(parent, alphaSliderTexture)
	...
	cpf.okButton = Phoenix_UIConfig:Button(cpf, 100, 20, OKAY);
	cpf.cancelButton = Phoenix_UIConfig:Button(cpf, 100, 20, CANCEL);
...
-- placeholder
function Phoenix_UIConfig:ColorPickerFrame(r, g, b, a, okCallback, cancelCallback, alphaSliderTexture, updateCallback)
...
local ColorInputMethods = {
	SetColor = function(self, color)
		-- Ensure color is a valid table
		if type(color) ~= "table" then
			color = {r = 1, g = 1, b = 1, a = 1}
		end
		
		-- Set default values for any missing components
		self.color.r = color.r or 1
		self.color.g = color.g or 1
		self.color.b = color.b or 1
		self.color.a = color.a or 1
		
		-- Use pcall to protect against errors
		pcall(function()
			self.target:SetVertexColor(self.color.r, self.color.g, self.color.b, self.color.a)
		end)
...
function Phoenix_UIConfig:ColorInput(parent, label, width, height, color, update, cancel)
	local button = CreateFrame('Button', nil, parent);
	button.Phoenix_UIConfig = self;
	
	self:SetObjSize(button, width or 100, height or 20);
	self:ApplyBackdrop(button, 'button', 'border');

	local target = button:CreateTexture(nil, "OVERLAY");
	target:SetTexture([[Interface\ChatFrame\ChatFrameColorSwatch]]);
	self:SetObjSize(target, 19, 19);
	target:SetPoint('LEFT', button, 'LEFT', 1, 0);
	...
	button:SetScript("OnClick", function(self)
		self.Phoenix_UIConfig:ColorPickerFrame(
			self.color.r,
			self.color.g,
			self.color.b,
			self.color.a,
