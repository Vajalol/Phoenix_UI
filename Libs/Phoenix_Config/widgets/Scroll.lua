...
Phoenix_UIConfig.ScrollBarEvents = {
...
Phoenix_UIConfig.ScrollFrameEvents = {
	...
	OnScrollRangeChanged = function(self, _, yRange)
		...
		scrollUpButton:Hide();
		--scrollUpButton:SetTexture([[Interface\Phoenix_UI\Libs\Phoenix_Config\media\ScrollUp]])
...
Phoenix_UIConfig.ScrollFrameMethods = {
	...
	UpdateSize     = function(self, newWidth, newHeight)
		...
		self.Phoenix_UIConfig:GlueAcross(self.scrollFrame, self, 2, -2, -self.scrollBarWidth - 2, 2);
		...
		if self.scrollChild then
			...
			-- Allow scroll child to be taller than the frame
			-- self.scrollChild:SetHeight(self.scrollFrame:GetHeight());
		end
	end,

	-- Calculate and update the scroll child height based on its content
	UpdateScrollChildHeight = function(self)
		...
		-- First set the scroll child to a reasonable starting height
		-- Make it 3x the frame height to ensure enough initial scrolling space
		local baseHeight = scrollFrameHeight * 3
		
		-- Find all elements in the scroll child
		local elements = {self.scrollChild:GetChildren()}
		local maxBottom = 0
		
		-- Calculate how much space the actual content takes
		for _, element in pairs(elements) do
			if element:IsShown() and element:IsVisible() then
				local _, y = element:GetPoint(1)
				if type(y) == "number" then
					local height = element:GetHeight() or 0
					local bottom = math.abs(y) + height + 20 -- Add padding
					maxBottom = math.max(maxBottom, bottom)
				end
			end
		end
		
		-- Use either the measured height or the base height, whichever is larger
		local totalHeight = math.max(baseHeight, maxBottom + 100)
...
function Phoenix_UIConfig:ScrollFrame(parent, width, height, scrollChild)
	...
	if not scrollChild then
		...
		-- Set initial height, but we'll update it later
		scrollChild:SetHeight(scrollFrame:GetHeight() * 3); -- Make it larger by default
	else
	...
	-- Add a function to recalculate scroll height when content changes
	panel.content = scrollChild;
	scrollChild.RecalculateHeight = function()
		panel:UpdateScrollChildHeight()
	end
	
	-- Hook the OnShow to update scroll child height
	...
	-- Schedule an update after everything is loaded
	C_Timer.After(0.5, function()
		panel:UpdateScrollChildHeight()
	end)

	-- Ensure mouse wheel scrolling works by setting up direct handlers
...
Phoenix_UIConfig.FauxScrollFrameMethods = {
	...
	Update           = function(self, numItems, numToDisplay, buttonHeight)
		...
		-- Handle the case where numToDisplay is a table (happens when passing options table)
		local displayCount = numToDisplay;
		if type(numToDisplay) == "table" then
			displayCount = self.displayCount; -- Use the frame's displayCount instead
		end

		if numItems > displayCount then
			showScrollBar = 1;
		else
			scrollBar:SetValue(0);
		end
		...
		if self:IsShown() then
			...
			if numItems > 0 then
				scrollFrameHeight = (numItems - displayCount) * buttonHeight;
			...
			local maxRange = (numItems - displayCount) * buttonHeight;
			if maxRange < 0 then
				maxRange = 0;
			end
			...
			scrollBar:SetStepsPerPage(displayCount - 1);
...
Phoenix_UIConfig.FauxScrollFrameEvents = {
...
--- of items
function Phoenix_UIConfig:FauxScrollFrame(parent, width, height, displayCount, lineHeight, scrollChild)
...
--- HybridScrollFrame
----------------------------------------------------
Phoenix_UIConfig.HybridScrollFrameMethods = {
	...
	CreateItems         = function(self, data, create, update, padding, oX, oY)
		...
		self.Phoenix_UIConfig:ObjectList(scrollChild, self.items, create, update, data, padding, oX, oY,
...
function Phoenix_UIConfig:HybridScrollFrame(parent, width, height, scrollChild)
	local panel = self:ScrollFrame(parent, width, height, scrollChild);
