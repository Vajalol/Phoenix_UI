		...
		for i = 1, #self.columns do
			...
			if col.header and StringLength(col.header) > 0 then
				if not self.headers[i] then
					self.headers[i] = {
						text = self.Phoenix_UIConfig:FontString(self, ''),
					};
				end
				...
				self.Phoenix_UIConfig:GlueTop(column.text, self, marginLeft, 0, 'LEFT');
	...
	DrawData = function(self)
		...
		for y = 1, #self.tableData do
			...
			for x = 1, #self.columns do
				...
				self.Phoenix_UIConfig:GlueTop(cell.text, self, marginLeft, marginTop, 'LEFT');
				marginLeft = marginLeft + col.width;
			end
...
function Phoenix_UIConfig:Table(parent, width, height, rowHeight, columns, data)
