...
local SimpleEditBoxMethods = {
	SetFontSize = function(self, newSize)
		self:SetFont(self:GetFont(), newSize, self.Phoenix_UIConfig.config.font.effect);
	end
};
...
--- @return EditBox
function Phoenix_UIConfig:SimpleEditBox(parent, width, height, text)
	--- @type EditBox
	local editBox = CreateFrame('EditBox', nil, parent);
	editBox.Phoenix_UIConfig = self;
...
function Phoenix_UIConfig:ApplyPlaceholder(widget, placeholderText, icon, iconColor)
	widget.placeholder = {};
...
function Phoenix_UIConfig:SearchEditBox(parent, width, height, placeholderText)
	...
	Validate = function(self)
		...
		if self.isValid then
			...
			if self.OnValueChanged and tostring(self.lastValue) ~= tostring(self.value) then
				...
				if Phoenix_UI and Phoenix_UI.SaveDB and not (Phoenix_UI.UI and Phoenix_UI.UI:IsVisible()) then
					-- Use a single delayed timer for all changes in this frame
					if not self.saveTimer then
						self.saveTimer = C_Timer.After(0.5, function()
							...
							if Phoenix_UI and Phoenix_UI.UI and Phoenix_UI.UI.RefreshConfig then
					...
					if Phoenix_UI and Phoenix_UI.UI and self.dbReference and self.dataKey then
						...
						if Phoenix_UIConfig and Phoenix_UIConfig.setDatabaseValue then
							Phoenix_UIConfig:setDatabaseValue(self.dbReference, self.dataKey, self.value)
						end
...
local EditBoxEvents = {
	OnEnterPressed = function(self)
		local value = Phoenix_UIConfig.Util.stripColors(self:GetText());
		self.value = value;
		self:Validate();
		self:ClearFocus();
	end,
	...
	OnEditFocusLost = function(self)
		-- When editbox loses focus, validate value automatically
		local value = Phoenix_UIConfig.Util.stripColors(self:GetText());
...
--- @return EditBox
function Phoenix_UIConfig:EditBox(parent, width, height, text, validator)
	validator = validator or Phoenix_UIConfig.Util.editBoxValidator;
...
function Phoenix_UIConfig:NumericBox(parent, width, height, text, validator)
...
local MoneyBoxMethods = {
	SetValue = function(self, value)
		self.value = value;
		local formatted = self.Phoenix_UIConfig.Util.formatMoney(value);
...
function Phoenix_UIConfig:MoneyBox(parent, width, height, text, validator, excludeCopper)
...
function Phoenix_UIConfig:MultiLineBox(parent, width, height, text, readOnly)
