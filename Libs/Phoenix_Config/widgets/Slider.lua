...
function Phoenix_UIConfig:SliderButton(parent, width, height, direction)
...
--- This is only useful for scrollBars not created using Phoenix_UIConfig
function Phoenix_UIConfig:StyleScrollBar(scrollBar)
	local buttonUp, buttonDown = scrollBar:GetChildren();

	scrollBar.background = Phoenix_UIConfig:Panel(scrollBar);
...
local SliderMethods = {
	...
	GetValue = function(self)
		...
		return Clamp(Phoenix_UIConfig.Util.roundPrecision(self:OriginalGetValue(), self.precision), minimum, maximum);
	end
};
...
function Phoenix_UIConfig:Slider(parent, width, height, value, vertical, min, max)
...
local SliderWithBoxMethods = {
	SetValue = function(self, v)
		...
		-- Get updated value from slider
		v = self.slider:GetValue();
		
		-- Update editbox with protection
		pcall(function()
...
function Phoenix_UIConfig:SliderWithBox(parent, width, height, value, min, max)
	local widget = CreateFrame('Frame', nil, parent);
	self:SetObjSize(widget, width, height);
	
	-- Ensure we have valid numeric values
	if type(value) ~= "number" then
		value = (min and type(min) == "number") and min or 0;
	end
...
function Phoenix_UIConfig:ScrollBar(parent, width, height, horizontal)
