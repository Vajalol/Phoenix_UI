...
Phoenix_UIConfig.Util.autocompleteTransformer = function(_, value)
	return value;
end

Phoenix_UIConfig.Util.autocompleteValidator = function(self)
	self.Phoenix_UIConfig:MarkAsValid(self, true);
	return true;
end

Phoenix_UIConfig.Util.autocompleteItemTransformer = function(_, value)
...
Phoenix_UIConfig.Util.autocompleteItemValidator = function(ac)
	...
	if itemId then
		ac.value = itemId;
		ac:SetText(itemName);
		self.Phoenix_UIConfig:MarkAsValid(ac, true);

		return true;
	else
		self.Phoenix_UIConfig:MarkAsValid(ac, false);
		return false;
	end
end
...
local AutocompleteMethods = {
	...
	buttonCreate = function(panel)
		...
		optionButton = Phoenix_UIConfig:HighlightButton(panel, panel:GetWidth(), 20, '');
		optionButton.highlight = Phoenix_UIConfig:HighlightButtonTexture(optionButton);
	...
	RenderItems = function(self)
		...
		self.Phoenix_UIConfig:ObjectList(
			self.dropdown,
			self.itemTable,
			self.buttonCreate,
			self.buttonUpdate,
...
local AutocompleteEvents = {
	...
	OnTextChanged = function(ac, isUserInput)
		local plainText = Phoenix_UIConfig.Util.stripColors(ac:GetText());
		ac.selectedItem = nil;
...
--- @return EditBox
function Phoenix_UIConfig:Autocomplete(parent, width, height, text, validator, transformer, items)
	transformer = transformer or Phoenix_UIConfig.Util.autocompleteTransformer;
	validator = validator or Phoenix_UIConfig.Util.autocompleteValidator;

	local autocomplete = self:EditBox(parent, width, height, text, validator);
	--@type Phoenix_UIConfig
	autocomplete.Phoenix_UIConfig = self;
