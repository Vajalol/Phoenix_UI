...
---@type ContextMenu
Phoenix_UIConfig.ContextMenuMethods = {
	...
	CreateItem        = function(parent, data, i)
		...
		if data.title then
			itemFrame = parent.Phoenix_UIConfig:Frame(parent, nil, 20);
			itemFrame.text = parent.Phoenix_UIConfig:Label(itemFrame);
			parent.Phoenix_UIConfig:GlueLeft(itemFrame.text, itemFrame, 0, 0, true);
		elseif data.isSeparator then
			itemFrame = parent.Phoenix_UIConfig:Frame(parent, nil, 20);
			itemFrame.texture = parent.Phoenix_UIConfig:Texture(itemFrame, nil, 8,
		...
		elseif data.checkbox then
			itemFrame = parent.Phoenix_UIConfig:Checkbox(parent, '');
		elseif data.radio then
			itemFrame = parent.Phoenix_UIConfig:Radio(parent, '', data.radioGroup);
		elseif data.text then
			itemFrame = parent.Phoenix_UIConfig:HighlightButton(parent, nil, 20);
		end
		...
		if not data.isSeparator and data.children then
			itemFrame.icon = parent.Phoenix_UIConfig:Texture(itemFrame, 10, 10, [[Interface\Buttons\SquareButtonTextures]]);
			itemFrame.icon:SetTexCoord(0.42187500, 0.23437500, 0.01562500, 0.20312500);
			parent.Phoenix_UIConfig:GlueRight(itemFrame.icon, itemFrame, -4, 0, true);

			itemFrame.childContext = parent.Phoenix_UIConfig:ContextMenu(parent, data.children, true, parent.level + 1);
	...
	UpdateItem        = function(parent, itemFrame, data, i)
		...
		if data.title then
			...
			parent.Phoenix_UIConfig:ButtonAutoWidth(itemFrame);
	...
	DrawOptions       = function(self, options)
		...
		local _, totalHeight = self.Phoenix_UIConfig:ObjectList(
			self,
			self.optionFrames,
			self.CreateItem,
			self.UpdateItem,
...
Phoenix_UIConfig.ContextMenuEvents = {
...
function Phoenix_UIConfig:ContextMenu(parent, options, stopHook, level)
	---@class ContextMenu
