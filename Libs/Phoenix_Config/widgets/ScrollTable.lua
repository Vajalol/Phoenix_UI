...
local methods = {
	...
	SetColumns        = function(self, columns)
		...
		for i = 1, #columns do
			...
			if not columnHeadFrame.columns[i] then
				columnFrame = self.Phoenix_UIConfig:HighlightButton(columnHeadFrame);
				columnFrame:SetPushedTextOffset(0, 0);

				columnFrame.arrow = self.Phoenix_UIConfig:Texture(columnFrame, 8, 8, [[Interface\Buttons\UI-SortArrow]]);
			...
			if align == 'LEFT' then
				...
				self.Phoenix_UIConfig:GlueRight(columnFrame.arrow, columnFrame, 0, 0, true);
			else
				columnFrame.arrow:ClearAllPoints();
				self.Phoenix_UIConfig:GlueLeft(columnFrame.arrow, columnFrame, 5, 0, true);
			end
		...
		for i = 1, numberOfRows do
			...
			for j = 1, #self.columns do
				...
				if not cell then
					...
					cell.text = self.Phoenix_UIConfig:FontString(cell, '');
	...
	SortData          = function(self, sortBy)
		-- Add a flag to prevent infinite recursion between SortData and Refresh
		...
		-- Create a temporary table to hold all the sorted values
		...
		TableSort(self.sortTable, function(rowIndexA, rowIndexB)
			if self.CompareSort then
				return self:CompareSort(rowIndexA, rowIndexB, sortBy);
			end
		end);

		self.filtered = self:DoFilter();
		
		-- Make sure to call Refresh outside of the current sort operation
		self.isSorting = false
		self:Refresh();
	...
	DoCellUpdate      = function(table, shouldShow, rowFrame, cellFrame, value, columnData, rowData, rowIndex)
		if shouldShow then
			...
			elseif format == 'money' then
				value = table.Phoenix_UIConfig.Util.formatMoney(value);
				cellFrame.text:SetText(value);
			elseif format == 'moneyShort' then
				value = table.Phoenix_UIConfig.Util.formatMoney(value, true);
				cellFrame.text:SetText(value);
			...
			elseif format == 'icon' then
				if cellFrame.texture then
					...
					cellFrame.texture = table.Phoenix_UIConfig:Texture(cellFrame, iconSize, iconSize, value);
			...
			elseif format == 'color' then
				if cellFrame.texture then
					...
					local iconSize = 10
					cellFrame.texture = table.Phoenix_UIConfig:Texture(cellFrame, iconSize, iconSize, [[Interface\Buttons\WHITE8X8]])
					cellFrame.texture:SetVertexColor(value.r, value.g, value.b, value.a)
					cellFrame.texture:SetPoint('CENTER', 0, 0)
				end
			elseif format == 'delete' then
				if not cellFrame.texture then
					local iconSize = 15
					cellFrame.texture = table.Phoenix_UIConfig:Texture(cellFrame, iconSize, iconSize, [[Interface\UITools\UITools]])
			...
			if color then
				...
				table.Phoenix_UIConfig:SetTextColor(cellFrame.text, 'normal');
			end

			if table.selectionEnabled then
				if table.selected == rowIndex then
					table:SetHighLightColor(rowFrame, table.Phoenix_UIConfig.config.highlight.color);
				else
	...
	Refresh           = function(self)
		...
		-- Set the offset of the scroll frame
		local offset = FauxScrollFrame_GetOffset(self.scrollFrame);
		local displayCount = self.numberOfRows;

		-- Make sure all data is sorted
		-- Add a flag to prevent infinite recursion between Refresh and SortData
		...
		FauxScrollFrame_Update(
			self.scrollFrame,
			#self.filtered,
			displayCount,  -- Pass number, not table
			rowHeight
		);
		...
		-- show selection if already selected
		if self.selectionRow then
			self:Select(self.selectionRow);
		end

		return offset + 1, offset + displayCount, displayCount;
	end,
...
function Phoenix_UIConfig:ScrollTable(parent, columns, numRows, rowHeight)
