...
--- @return Frame
function Phoenix_UIConfig:Window(parent, width, height, title)
	...
	if frame.DisableHelpButton then
		frame:DisableHelpButton();
	end
	
	-- Make the window not show up in the UI panel system
	...
	function frame:MakeResizable(direction)
		Phoenix_UIConfig:MakeResizable(frame, direction);
		return frame;
	end
...
-- Reusing dialogs
Phoenix_UIConfig.dialogs = {};
--- @return Frame
function Phoenix_UIConfig:Dialog(title, message, dialogId)
	local window;
...
--- }
--- @return Frame
function Phoenix_UIConfig:Confirm(title, message, buttons, dialogId)
